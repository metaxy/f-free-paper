...
\label{sec:problem}
Um Heuristiken für das Entfernen von verbotenen Teilgraphen entwickeln zu können, ist zuerst das Problem selbst zu definieren. 
Es ist unter dem Namen \textsc{\cursive{F}-Free Edge Editing} bekannt.
\begin{quote}
  ...
  \textbf{Parameter:} $k$
\end{quote}
Es gibt auch das \textsc{F-Free Edge Editing}. Hier ist \textsc{F} keine Menge von Graphen, sondern nur ein Graph. In dieser Arbeit werden wir uns darauf beschränken, dass \cursive{F} eine endliche Menge ist.\\
...
\label{sec:examples}
In diesem Abschnitt werden wir auf einige Anwendungsfälle eingehen, warum Graphen mit bestimmten Eigenschaften gewünscht sind.
...
Nun ist \textsc{\cursive{F}-Free Edge Editing} für die meisten \cursive{F} sehr aufwendig zu lösen \cite{Burzyn06}. Nun ist es möglich dieses Problem nicht perfekt zu lösen indem wir die minimale Anzahl von Änderungen finden, sonder einfach eine möglichst kleine Anzahl von Änderungen.
